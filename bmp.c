...
int quant_error(unsigned char color){
    unsigned short bit_16 = color >> 3;
    unsigned short bit_24 = bit_16 * (255.0 / 31);
    int diff = (int)color - (int)bit_24;
    return(diff);
}
    ...
    for(i = conv_image->header.height - 1; i >= 0; i--){
        ...
        for(j = 0; j < conv_image->header.width * 3; j++){
            ...
            if(j < conv_image->header.width * 3 - 3){
                q_error[i][j + 3] += 7 * pixel_error; 
            }
            if(j < conv_image->header.width * 3 - 3 && i > 0){
                q_error[(i - 1)][j + 3] += pixel_error; 
            }
            if(i > 0){
                q_error[(i - 1)][j] += 5 * pixel_error; 
            }
            if(j > 2 && i > 0){
                q_error[(i - 1)][j-3] += 3 * pixel_error; 
            }
